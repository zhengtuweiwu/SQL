...
if __name__ == '__main__':
    ...
    sqlLength = 80
    # 训练脚本路径及文件名
    sql_filepath = "tf_data\\sql-train-80.tf"
    ...
    epochs = 80000
    # 测试脚本路径及文件名
    test_sql_filepath = "test_data\\sql-test-80.txt"
    ...
    saver.save(sess, "model\\GRU\\sql-cnn-" + str(sqlLength) + "\\sql-cnn-" + str(sqlLength))
    ###########以下为现场测试
    ...
    with tf.Session() as sess_test:
        new_saver = tf.train.import_meta_graph(
            "model\\GRU\\sql-cnn-" + str(sqlLength) + "\\sql-cnn-" + str(sqlLength) + ".meta")
        new_saver.restore(sess_test, tf.train.latest_checkpoint("model\\GRU\\sql-cnn-" + str(sqlLength)))
