...
if __name__ == '__main__':
    ...
    sqlLength=400
    #训练脚本路径及文件名
    sql_filepath="tf_data\\sql-train-400.tf"
    ...
    epochs=260000
    # 测试脚本路径及文件名
    test_sql_filepath = "test_data\\sql-test-400.txt"
    ...
    X_INPUT = tf.reshape(X_INPUT, [-1, 1, 2 * sqlLength, 1])
    #print(X)
    ...
    h_pool2_flat = tf.reshape(h_pool2, [-1, 1 * (int(sqlLength/2)) * 64])

    # fc1
    #W_fc1 = weight_variable([7 * 7 * 64, 1024])
    W_fc1 = weight_variable([1 * (int(sqlLength/2)) * 64, 1024])
    ...
    saver.save(sess, "model\\sql-cnn-"+str(sqlLength)+"\\sql-cnn-"+str(sqlLength))
    ###########以下为现场测试
    '''
    data_val1, label_val1 = sess.run([data_batch1, label_batch1])
    data_val2, label_val2 = sess.run([data_batch2, label_batch2])
    ...
    with tf.Session() as sess_test:
        new_saver = tf.train.import_meta_graph( "model\\sql-cnn-"+str(sqlLength)+"\\sql-cnn-"+str(sqlLength)+".meta")
        new_saver.restore(sess_test, tf.train.latest_checkpoint("model\\sql-cnn-"+str(sqlLength)))
