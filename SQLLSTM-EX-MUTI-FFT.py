...
if __name__ == '__main__':
    ...
    outputs, state = tf.nn.dynamic_rnn(lstm_cell_b, inputs=X, initial_state=None, time_major=False,dtype=tf.float32)#如果加载此处参数，容易固化网络输入状态，导致智能输入batch数量参数检测,坑点！！！
    #第一层输出结果
    ...
    # 单独再构建一遍FFT网络##########################
    #################################################
    lstm_cell_b_fft = rnn.BasicLSTMCell(num_units=hidden_size, forget_bias=1.0, state_is_tuple=True,reuse=True)
    lstm_cell_b_fft = rnn.DropoutWrapper(cell=lstm_cell_b_fft, input_keep_prob=1.0, output_keep_prob=keep_prob)
    outputs_fft, state_fft = tf.nn.dynamic_rnn(lstm_cell_b_fft, inputs=X_FFT, initial_state=None, time_major=False,dtype=tf.float32)
    h_state_fft = outputs_fft[:, -1, :]  # 或者 h_state = state[-1][1]
    if layer_num >= 2:
        s_X_fft = tf.reshape(h_state_fft, [-1, 1, hidden_size])
        lstm_cell_u_fft = rnn.BasicLSTMCell(num_units=hidden_size, forget_bias=1.0, state_is_tuple=True,reuse=True)
        lstm_cell_u_fft = rnn.DropoutWrapper(cell=lstm_cell_u_fft, input_keep_prob=1.0, output_keep_prob=keep_prob)
        mlstm_cell_fft = rnn.MultiRNNCell(cells=[lstm_cell_u_fft] * (layer_num - 1), state_is_tuple=True)
        outputs_fft, state_fft = tf.nn.dynamic_rnn(mlstm_cell_fft, inputs=s_X_fft, initial_state=None,time_major=False, dtype=tf.float32)
        h_state_fft = outputs_fft[:, -1, :]  # 或者 h_state = state[-1][1]
    ...
    saver.save(sess, "model\\LSTM\\MUTI-FFT\\sql-ex-cnn-" + str(sqlLength) + "\\sql-ex-cnn-" + str(sqlLength))
    ###########以下为现场测试
    ...
    with tf.Session() as sess_test:
        new_saver = tf.train.import_meta_graph(
            "model\\LSTM\\MUTI-FFT\\sql-ex-cnn-" + str(sqlLength) + "\\sql-ex-cnn-" + str(sqlLength) + ".meta")
        new_saver.restore(sess_test, tf.train.latest_checkpoint("model\\LSTM\\MUTI-FFT\\sql-ex-cnn-" + str(sqlLength)))
