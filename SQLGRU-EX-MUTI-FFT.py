...
if __name__ == '__main__':
    ...
    ####################################################################
    # **步骤1：完成双节点GRU两个输入的构建，分别是原始数据和FFT转换数据
    X = tf.reshape(X_, [-1, 1, sqlLength], name="source_input")
    X_FFT = tf.cast(X_, tf.complex64)
    X_FFT = tf.fft(X_FFT)
    X_FFT = tf.cast(X_FFT, tf.float32)
    X_FFT = tf.reshape(X_FFT, [-1, 1,sqlLength], name="fft_input")


    # **步骤2：分别定义第一层 GRU_cell，只需要说明 hidden_size, 它会自动匹配输入的 X和X_FFT 的维度，注意由于GRU单元是共享参数的，需要分别定义网络
    gru_cell_b = rnn.GRUCell(num_units=hidden_size)
    # **步骤3：添加 dropout layer, 一般只设置 output_keep_prob
    gru_cell_b = rnn.DropoutWrapper(cell=gru_cell_b, input_keep_prob=1.0, output_keep_prob=keep_prob)
    # **步骤4：初始化第一层 并且单独输出结果，并且使其运行起来
    #init_state = gru_cell_b.zero_state(batch_size, dtype=tf.float32)#如果加载此处参数，容易固化网络输入状态，导致智能输入batch数量参数检测,坑点！！！
    outputs, state = tf.nn.dynamic_rnn(gru_cell_b, inputs=X, initial_state=None, time_major=False, dtype=tf.float32)#如果加载此处参数，容易固化网络输入状态，导致智能输入batch数量参数检测,坑点！！！
    ...
    if layer_num >= 2:
        ...
        # **步骤5：定义顶层GRU_cell，可以配置多层
        gru_cell_u = rnn.GRUCell(num_units=hidden_size)
        # **步骤6：添加 dropout layer, 一般只设置 output_keep_prob
        gru_cell_u = rnn.DropoutWrapper(cell=gru_cell_u, input_keep_prob=1.0, output_keep_prob=keep_prob)
        # **步骤7：调用 MultiRNNCell 来实现顶层的多层 GRU
        mgru_cell = rnn.MultiRNNCell(cells=[gru_cell_u] * (layer_num - 1), state_is_tuple=True)
        # **步骤8：方法一，调用 dynamic_rnn() 来让我们构建好的网络运行起来
        # ** 当 time_major==False 时， outputs.shape = [batch_size, timestep_size, hidden_size]
        # ** 所以，可以取 h_state = outputs[:, -1, :] 作为最后输出
        # ** state.shape = [layer_num, 2, batch_size, hidden_size],
        # ** 或者，可以取 h_state = state[-1][1] 作为最后输出
        # ** 最后输出维度是 [batch_size, hidden_size]
        outputs, state = tf.nn.dynamic_rnn(mgru_cell, inputs=s_X, initial_state=None, time_major=False,dtype=tf.float32)#如果加载此处参数，容易固化网络输入状态，导致智能输入batch数量参数检测,坑点！！！
        h_state = outputs[:, -1, :]  # 或者 h_state = state[-1][1]

    # 单独再构建一遍FFT网络##########################
    #################################################
    gru_cell_b_fft = rnn.GRUCell(num_units=hidden_size,reuse=True)
    gru_cell_b_fft = rnn.DropoutWrapper(cell=gru_cell_b_fft, input_keep_prob=1.0, output_keep_prob=keep_prob)
    outputs_fft, state_fft = tf.nn.dynamic_rnn(gru_cell_b_fft, inputs=X_FFT, initial_state=None, time_major=False,dtype=tf.float32)
    h_state_fft = outputs_fft[:, -1, :]  # 或者 h_state = state[-1][1]
    if layer_num >= 2:
        s_X_fft = tf.reshape(h_state_fft, [-1, 1, hidden_size])
        gru_cell_u_fft = rnn.GRUCell(num_units=hidden_size,reuse=True)
        gru_cell_u_fft = rnn.DropoutWrapper(cell=gru_cell_u_fft, input_keep_prob=1.0, output_keep_prob=keep_prob)
        mgru_cell_fft = rnn.MultiRNNCell(cells=[gru_cell_u_fft] * (layer_num - 1), state_is_tuple=True)
        outputs_fft, state_fft = tf.nn.dynamic_rnn(mgru_cell_fft, inputs=s_X_fft, initial_state=None, time_major=False,dtype=tf.float32)
        h_state_fft = outputs_fft[:, -1, :]  # 或者 h_state = state[-1][1]
    #两个网络输出结果进行拼接合并作为下一步全连接网络的输入
    #out_y = tf.reshape(h_state, [-1, 1, hidden_size])
    #out_y_fft = tf.reshape(h_state_fft, [-1, 1, hidden_size])
    outresult=tf.concat([h_state, h_state_fft],1)
    # *************** 为了更好的理解 GRU工作原理，我们把上面 步骤6 中的函数自己来实现 ***************
    ...
    # 开始训练和测试
    W = tf.Variable(tf.truncated_normal([2*hidden_size, 2], stddev=0.1), dtype=tf.float32)
    bias = tf.Variable(tf.constant(0.1, shape=[2]), dtype=tf.float32)
    y_pre = tf.nn.softmax(tf.matmul(outresult, W) + bias,name="out")
    ...
    saver.save(sess, "model\\GRU\\MUTI-FFT\\sql-ex-cnn-" + str(sqlLength) + "\\sql-ex-cnn-" + str(sqlLength))
    ###########以下为现场测试
    ...
    with tf.Session() as sess_test:
        new_saver = tf.train.import_meta_graph(
            "model\\GRU\\MUTI-FFT\\sql-ex-cnn-" + str(sqlLength) + "\\sql-ex-cnn-" + str(sqlLength) + ".meta")
        new_saver.restore(sess_test, tf.train.latest_checkpoint("model\\GRU\\MUTI-FFT\\sql-ex-cnn-" + str(sqlLength)))
