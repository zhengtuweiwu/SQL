...
if __name__ == '__main__':
    ...
    # 把X_转为卷积所需要的形式，此处为FFT数据网络构建
    X_FFT = tf.cast(X_, tf.complex64)
    X_FFT = tf.fft(X_FFT)
    X_FFT = tf.cast(X_FFT, tf.float32)
    X_FFT = tf.reshape(X_FFT, [-1, 1, sqlLength, 1], name="fft_input")
    # print(X)
    # 第一层卷积：1×5×1卷积核32个 [1，5，1，32],h_conv1.shape=[-1, 1, sqlLength, 32]
    W_conv1_fft = weight_variable([1, stride, 1, 32])
    b_conv1_fft = bias_variable([32])
    h_conv1_fft = tf.nn.relu(conv2d(X_FFT, W_conv1_fft) + b_conv1_fft)
    # 第一个pooling 层[-1, 1, sqlLength, 32]->[-1, 1, sqlLength/2, 32]
    h_pool1_fft = max_pool_2x2(h_conv1_fft)
    # 第二层卷积：1×5×32卷积核64个 [1，5，32，64],h_conv2.shape=[-1, 1, sqlLength/2, 64]
    W_conv2_fft = weight_variable([1, stride, 32, 64])
    b_conv2_fft = bias_variable([64])
    h_conv2_fft = tf.nn.relu(conv2d(h_pool1_fft, W_conv2_fft) + b_conv2_fft)
    # 第二个pooling 层,[-1, 1, sqlLength/2, 64]->[-1, 1, sqlLength/4, 64]
    h_pool2_fft = max_pool_2x2(h_conv2_fft)
    # flatten层，[-1, 1, sqlLength/4, 64]->[-1, 7*(sqlLength/4)*64],即每个样本得到一个1 * (sqlLength/4) * 64维的样本
    h_pool2_flat_fft = tf.reshape(h_pool2_fft, [-1, 1 * (int(sqlLength / 4)) * 64])
    #W_fc1 = weight_variable([7 * 7 * 64, 1024])
    W_fc1_fft = weight_variable([1 * (int(sqlLength/4)) * 64, 1024])
    b_fc1_fft = bias_variable([1024])
    h_fc1_fft = tf.nn.relu(tf.matmul(h_pool2_flat_fft, W_fc1_fft) + b_fc1_fft)
    # dropout: 输出的维度和h_fc1一样，只是随机部分值被值为零
    h_fc1_drop_fft = tf.nn.dropout(h_fc1, keep_prob)
    # 汇总两个卷积网络的全连接层数据进入输入层
    out_drop = tf.concat([h_fc1_drop, h_fc1_drop_fft], 1)
    W_fc2 = weight_variable([2048, 2])
    b_fc2 = bias_variable([2])
    y_conv = tf.nn.softmax(tf.matmul(out_drop, W_fc2) + b_fc2,name="out")
    ...
    saver.save(sess, "model\\CNN-FFT\\MUTI\\sql-ex-cnn-" + str(sqlLength) + "\\sql-ex-cnn-" + str(sqlLength))
    ###########以下为模型现场测试
    ...
    with tf.Session() as sess_test:
        new_saver = tf.train.import_meta_graph("model\\CNN-FFT\\MUTI\\sql-ex-cnn-"+str(sqlLength)+"\\sql-ex-cnn-"+str(sqlLength)+".meta")
        new_saver.restore(sess_test, tf.train.latest_checkpoint("model\\CNN-FFT\\MUTI\\sql-ex-cnn-"+str(sqlLength)))
