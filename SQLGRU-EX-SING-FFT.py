...
if __name__ == '__main__':
    ...
    outputs, state = tf.nn.dynamic_rnn(gru_cell_b, inputs=X_INPUT, initial_state=None, time_major=False,dtype=tf.float32)#如果加载此处参数，容易固化网络输入状态，导致智能输入batch数量参数检测,坑点！！！
    # 第一层输出结果
    ...
    saver.save(sess, "model\\GRU\\SINGAL-FFT\\3layers\\sql-ex-cnn-" + str(sqlLength) + "\\sql-ex-cnn-" + str(sqlLength))
    ###########以下为现场测试
    ...
    with tf.Session() as sess_test:
        new_saver = tf.train.import_meta_graph(
            "model\\GRU\\SINGAL-FFT\\3layers\\sql-ex-cnn-" + str(sqlLength) + "\\sql-ex-cnn-" + str(sqlLength) + ".meta")
        new_saver.restore(sess_test, tf.train.latest_checkpoint("model\\GRU\\SINGAL-FFT\\3layers\\sql-ex-cnn-" + str(sqlLength)))
