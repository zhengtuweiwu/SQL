...
if __name__ == '__main__':
    ...
    saver.save(sess, "model\\CNN-FFT\\MUTI\\sql-cnn-"+str(sqlLength)+"\\sql-cnn-"+str(sqlLength))
    ###########以下为现场测试
    '''
    data_val1, label_val1 = sess.run([data_batch1, label_batch1])
    data_val2, label_val2 = sess.run([data_batch2, label_batch2])
    ...
    with tf.Session() as sess_test:
        new_saver = tf.train.import_meta_graph( "model\\CNN-FFT\\MUTI\\sql-cnn-"+str(sqlLength)+"\\sql-cnn-"+str(sqlLength)+".meta")
        new_saver.restore(sess_test, tf.train.latest_checkpoint("model\\CNN-FFT\\MUTI\\sql-cnn-"+str(sqlLength)))
