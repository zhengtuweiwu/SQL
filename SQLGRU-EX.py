...
if __name__ == '__main__':
    ...
    sqlLength = 80
    # 训练脚本路径及文件名
    sql_filepath = "tf_data\\sql-ex-train-80.tf"
    normal_filepath = "tf_data\\normal-ex-train-80.tf"
    # 训练batch长度
    batch_size = 128
    # 迭代次数
    epochs =40000
    # 测试脚本路径及文件名
    test_sql_filepath = "test_data\\sql-ex-test-80.txt"
    ...
    # **步骤2：定义一层 GRU_cell，只需要说明 hidden_size, 它会自动匹配输入的 X 的维度
    ...
    if layer_num >= 2:
        ...
        #init_state = mgru_cell.zero_state(batch_size, dtype=tf.float32)#如果加载此处参数，容易固化网络输入状态，导致智能输入batch数量参数检测,坑点！！！
    ...
    saver.save(sess, "model\\GRU\\NO-FFT\\"+str(hidden_size)+"port\\sql-ex-cnn-" + str(sqlLength) + "\\sql-ex-cnn-" + str(sqlLength))
    ###########以下为现场测试
    ...
    with tf.Session() as sess_test:
        ...
        new_saver.restore(sess_test, tf.train.latest_checkpoint("model\\GRU\\NO-FFT\\"+str(hidden_size)+"port\\sql-ex-cnn-" + str(sqlLength)))
